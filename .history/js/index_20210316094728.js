...
const handleVoice = (text) => {
  ...
  if (handledText.includes('change color')) {
    const color = handledText.split('color')[1].trim();
    container.style.background = color;
    return;
  }

  if (handledText.includes('default color')) {
  ...
  if (handledText.includes('what time')) {
    ...
    const formattedTime = moment(`${hour}:${minute}, hh:mm A`).format('hh:mm A');
    console.log(formattedTime);
    const textToSpeech = `${moment().hours()} ${moment().minutes()}`;
    speak(textToSpeech);
    return;
  }
